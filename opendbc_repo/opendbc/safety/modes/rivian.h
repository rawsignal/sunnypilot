...
static void rivian_rx_hook(const CANPacket_t *msg) {
  ...
  if (msg->bus == 1U) {
    // Cruise state (ACM_Status on bus 1 for cam-dev tap)
    if (msg->addr == 0x100U) {
...
static bool rivian_tx_hook(const CANPacket_t *msg) {
  ...
  // Fault avoidance blip: zero torque during blip, snap back to same value after. max_rate_up=310 allows 0->stored snap (matches low speed torque limit); normal op rate-limited to 3 by car controller.
  const TorqueSteeringLimits RIVIAN_STEERING_LIMITS = {
    .max_torque = 310,  // low speed
    .dynamic_max_torque = true,
    .max_torque_lookup = {
      {9., 17., 17.},
      {310, 250, 250},
    },
    .max_rate_up = 310,   // allow 0->stored snap after blip
    .max_rate_down = 5,
    .max_rt_delta = 310,  // allow snap within 250ms window
...
static safety_config rivian_init(uint16_t param) {
  ...
  static RxCheck rivian_rx_checks[] = {
    ...
    {.msg = {{0x102, 0, 8, 50U, .ignore_checksum = true, .ignore_counter = true, .ignore_quality_flag = true}, { 0 }, { 0 }}},   // ESP_AebFb (brakes, cam-dev tap)
